...
and row entrances, then render them into a KML with distinct colors and a
...
def extract_tree_and_row_waypoints(
    ...
    for data in payloads:
        ...
        row_entrances = data.get("row_entrances")
        if isinstance(row_entrances, list):
            for entrance in row_entrances:
    ...
    return best_points, perimeter_waypoints
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Convert .bin (length-prefixed JSON) to KML with tree points and row entrances."
    )
    ...
    tree_points, perimeter_waypoints = extract_tree_and_row_waypoints(payloads)
    if not tree_points and not perimeter_waypoints:
        raise SystemExit("No tree points or row entrances found in input.")

    all_points = collect_all_points(tree_points, perimeter_waypoints)
